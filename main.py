...
###########################################################################################################
    ...
    window.configure(background="#051730")
...
###########################################################################################################
    ...
    btn_end.place(x=260, y=200)                          #End Button
###########################################################################################################
    ttl = tk.Text(window, height=1, width=16, bg="#051730", fg="white", font=("Comic Sans", 16, "bold"))
    ttl.tag_configure("center", justify="center")        #Title
    ttl.place(x=110, y=25)
    ttl.insert(tk.END, "Fisch Snail Macro", "center")
###########################################################################################################
    credits = tk.Text(window, height=1, width=16, bg="#051730", fg="light grey", font=("Comic Sans", 9, "bold"))
    credits.tag_configure("center", justify="center")   #Credits
    credits.place(x=150, y=70)
    credits.insert(tk.END, "By @glitch54932", "center")
###########################################################################################################
    window.mainloop()
...
if "__main__" == __name__:
    ...
    listener = keyboard.Listener(on_press=start)          #Starts key listener for F8 (start)
    listener2 = keyboard.Listener(on_press=quit)          #Starts key listener for F7 (end)
    listener.start()
    listener2.start()
